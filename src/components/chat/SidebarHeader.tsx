import { Loader2, Plus, Cloud, Inbox, ClipboardList } from 'lucide-react';
...
export const SidebarHeader = ({
  ...
  const toggleMyTasks = useMyTasksStore((state) => state.togglePanel);
    ...
    <div className="h-[60px] flex items-center justify-between px-4 shrink-0 border-b border-indigo-100/70 dark:border-zinc-800/60">
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-100/80 text-indigo-600 transition-all duration-200 hover:bg-indigo-600 hover:text-white dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500"
          title="Task của tôi"
          aria-label="Task của tôi"
        >
