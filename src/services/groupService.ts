...
export const groupService = {
  ...
  async createChannel(groupId: string, data: { name: string; type?: string; isPrivate?: boolean; isTaskEnabled?: boolean; isPostingRestricted?: boolean; memberIds?: string[] }): Promise<ApiResponse<ChannelResponse>> {
    const response = await apiClient.post<ApiResponse<ChannelResponse>>(`/groups/${groupId}/channels`, data);
    return response.data;
  },

  async updateChannel(groupId: string, channelId: string, data: { name?: string; type?: string; isPrivate?: boolean; isTaskEnabled?: boolean; isPostingRestricted?: boolean; memberIds?: string[] }): Promise<ApiResponse<ChannelResponse>> {
